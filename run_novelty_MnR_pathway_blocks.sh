...
DIRS=(
  "Simulation_Results/2D_box_Familiar2Novelty/Silenced_MnR_to_VCpvm/Trial_${BASE_TRIAL}/Run_${RUN_0}"
  "Simulation_Results/2D_box_Familiar2Novelty/Silenced_MnR_to_CCK_VCnvm/Trial_${BASE_TRIAL}/Run_${RUN_0}"
)
...
# --- Condition 0: Novelty (Silenced MnR to VCpvm) ---
echo "--------------------------------------------------------"
echo "Running Novelty condition with MnR silenced to VCpvm (Silenced_MnR_to_VCpvm)..."
run_sim -c n_runs=$RUN_0 -c n_trials=$BASE_TRIAL -c n_neuron=10 -c learning_cond=4

# --- Condition 0: Novelty (Silenced MnR to CCK and VCnvm) ---
echo "--------------------------------------------------------"
echo "Running Novelty condition with MnR silenced to CCK and VCnvm (Silenced_MnR_to_CCK_VCnvm)..."
run_sim -c n_runs=$RUN_0 -c n_trials=$BASE_TRIAL -c n_neuron=11 -c learning_cond=4
